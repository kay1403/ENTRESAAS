...
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  TEST DES RÔLES AVEC PERMISSIONS JSON${NC}"
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}\n"
...
# Test 1: Création rôle simple (sans permissions)
echo -e "${YELLOW}📝 Test 1: Création rôle simple...${NC}"
...
# Test 2: Création rôle avec description
echo -e "${YELLOW}📝 Test 2: Création rôle avec description...${NC}"
ROLE2=$(curl -s -X POST "$BASE_URL/roles" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "TEST_ROLE_2",
    "description": "Rôle de test avec description"
  }')
...
# Test 3: Création rôle avec permissions personnalisées
...
ROLE3=$(curl -s -X POST "$BASE_URL/roles" \
  ...
  -d '{
    "name": "TEST_ROLE_3",
    "description": "Rôle avec permissions",
    "permissions": {
      "users": {
        "read": true,
        "create": true,
...
if [[ $ROLES == *"TEST_ROLE_3"* ]]; then
  echo -e "${GREEN}✅ TEST_ROLE_3 trouvé${NC}"
fi
echo ""
...
[ ! -z "$ROLE3_ID" ] && curl -s -X DELETE "$BASE_URL/roles/$ROLE3_ID" -H "Authorization: Bearer $ADMIN_TOKEN" > /dev/null && echo -e "${GREEN}✅ Rôle 3 supprimé${NC}"

echo -e "\n${GREEN}════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  ✅ TESTS JSON TERMINÉS${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
