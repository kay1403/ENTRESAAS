...
export default function AuditLogsPage() {
  ...
  const handleExport = async () => {
    try {
      ...
      showToast.success('Logs exportés avec succès');
    } catch (error) {
      showToast.error('Échec de l\'export');
    }
  };
        ...
        <div className="mb-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 flex items-center space-x-2">
              ...
              <input
                type="text"
                placeholder="Filtrer par action, utilisateur ou IP..."
                value={filter}
                onChange={(e) => setFilter(e.target.value)}
                className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-600"
              />
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">Aucun log trouvé</h3>
              <p className="mt-2 text-gray-600 dark:text-gray-400">
                {filter || dateRange.start || dateRange.end 
                  ? 'Essayez d\'ajuster vos filtres' 
                  : 'Les logs d\'audit apparaîtront ici'}
              </p>
            </div>
          ) : (
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                  {filteredLogs.map((log) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                        {log.user?.email || `Utilisateur #${log.userId}`}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                        {log.ip || 'N/A'}
                      </td>
                      <td className="px-6 py-4 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800">
                        Voir détails →
                      </td>
                    </tr>
                  ))}
      ...
      {selectedLog && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full p-6 animate-slideIn">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Détails du log</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Utilisateur</label>
                <p className="mt-1 text-gray-900 dark:text-white">{selectedLog.user?.email || `Utilisateur #${selectedLog.userId}`}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Adresse IP</label>
                <p className="mt-1 text-gray-900 dark:text-white">{selectedLog.ip || 'N/A'}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Date</label>
                <p className="mt-1 text-gray-900 dark:text-white">{new Date(selectedLog.createdAt).toLocaleString('fr-FR')}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Données</label>
